...
function AuthProvider(props: AppBaseProviderProps) {
  ...
  const getUserAuthDataFromStorage =
    useCallback<GetUserAuthDataFromStorageFunc>(() => {
  ...
  const signIn = useCallback<SignInAsyncFunc>(async (signIn: SignInModel) => {
    ...
    try {
      ...
      console.log(
        `The authentication process was failed with error: ${(error as Error).message}`
      );
      throw error;
    }
  }, []);
  ...
  const signOut = useCallback<SignOutAsyncFunc>(async () => {
    ...
    if (userAuthData) {
      try {
        ...
        console.log(
          "It was happened error during a process of an user security token revoke!"
        );
        throw error;
      }
    }
